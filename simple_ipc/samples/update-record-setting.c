/**
 * this sample code implement main bitstream + sub1 bitstream + sub2 bitstream update record function  
 * after record 10 seconds, update main bitstream record.
 * after record 15 seconds, update sub1 bitstream record.
...
int main(int argc, char *argv[])
{
    ...
    sub_update.update_parm.src.di_mode = LVFRAME_EVEN_ODD;
    sub_update.update_parm.src.mode = LVFRAME_FRAME_MODE;
    sub_update.update_parm.src.scale_indep = CAPSCALER_NOT_KEEP_RATIO;
    ...
    while(1) {          
            ...
            if ((t2.tv_sec - t1.tv_sec) == 10) {    //<record for 10 seconds. then update record setting
                if(flag == 0){
                   flag = 1;
            ...
            if ((t2.tv_sec - t1.tv_sec) == 15) {    //<record for 15 seconds. then update sub bitstream record setting
                if(sub_flag == 0){
                    sub_flag = 1;                    
                    sub_update.command = ENC_UPDATE;
                    ioctl(enc_fd, DVR_ENC_CONTROL, &sub_update);
            ...
            if ((t2.tv_sec - t1.tv_sec) == 25) {    //<record for 25 seconds. then finish record.
                fclose(rec_file);      
                fclose(rec_sub_bs_file[sub1_num - 1]);       
