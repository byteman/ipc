/**
 * this sample code implement MPEG4 motion detection function without ISP module, and record for 20 seconds.
 *
 */
...
dvr_enc_channel_param   user_rec_ch_setting = 
{
    ...
    },
    {
        ...
        DVR_ENC_EBST_ENABLE, 
        {1280, 720},         
        {ENC_INPUT_H2642D, 30, 1048576,  30, 25, 31, 1 , FALSE, {0, 0, 320, 240}},   
...
int main(int argc, char *argv[])
{
    ...
    fv_motion_info_init(&md_param, &active, &gfvcmd, mb_width, mb_height);
    ...
    for (count = 0; count < 1; count ++) {   
        ...
        for (i = 0; i < mb_height; i++)
            for (j = 0; j < mb_width; j++)
                md_param.mb_cell_th[(i*mb_width + j)] = 25;    
    }        
    ...
    while(1) {          
        ...
        //do motion detection
        for (count = 0; count < 1; count ++) {
            fv_motion_detection((MBK_INFO *)(bs_buf + data.bs.mv_offset), &md_param, &active, &gfvcmd);                       
